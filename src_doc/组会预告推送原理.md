...
组会表里的 `顺序` 字段是人工维护的，**它就是预告里的 Track 号**，不做密集重编号：

```
原始顺序 [3, 1]   →  按顺序排列  →  Track 1, 3   （跳号保留）
原始顺序 [2, 2]   →  按顺序排列  →  Track 1, 2   （重复的让路）
原始顺序 [1, 空]  →  按顺序排列  →  Track 1, 2   （空缺补位）
```

廖剑雄这周 `顺序 = 3`、且本周只有他一位报告人时，预告里就该是 `Track 3`，而不是被压成 `Track 1`。跳号是正常的，别把它「修」回来。

只有两种值会被改写：

- **空缺或非数字**（`顺序` 是富文本单元格时也按文本取第一个数字）→ 补到当前场次里最小的空闲号。
